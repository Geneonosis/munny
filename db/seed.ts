...
export async function seed() {
  ...
  for (const name of SYSTEM_CATEGORIES) {
    db.run(sql`INSERT OR IGNORE INTO ${categories} (name, is_system) VALUES (${name}, 1)`);
  }
...
export async function seedDev() {
  ...
  for (const bucket of mockBuckets) {
    const exists = db.select({ id: buckets.id }).from(buckets)
      .where(sql`name = ${bucket.name} AND type_id = ${bucket.typeId}`).get();
    if (!exists) {
  ...
  const mockEntries = [
    // Chase Checking
    { bucketId: bucketByName["Chase Checking"], amount: 250000, flow: "in",  note: "Opening deposit",      date: "2026-01-01", category: "income" },
    { bucketId: bucketByName["Chase Checking"], amount:   8500, flow: "out", note: "Grocery run",          date: "2026-01-05", category: "groceries" },
    { bucketId: bucketByName["Chase Checking"], amount:  15000, flow: "out", note: "Electric bill",        date: "2026-01-10", category: "utilities" },
    { bucketId: bucketByName["Chase Checking"], amount: 320000, flow: "in",  note: "Paycheck",             date: "2026-02-01", category: "income" },
    { bucketId: bucketByName["Chase Checking"], amount:  12000, flow: "out", note: "Internet + streaming", date: "2026-02-03", category: "utilities" },
    // Chase Savings
    { bucketId: bucketByName["Chase Savings"],  amount: 500000, flow: "in",  note: "Initial transfer",     date: "2026-01-01", category: "transfer" },
    { bucketId: bucketByName["Chase Savings"],  amount:  50000, flow: "in",  note: "Monthly contribution", date: "2026-02-01", category: "savings & investment" },
    // Amex Gold
    { bucketId: bucketByName["Amex Gold"],      amount:  24999, flow: "out", note: "Restaurant dinner",    date: "2026-01-15", category: "food & dining" },
    { bucketId: bucketByName["Amex Gold"],      amount:  59900, flow: "out", note: "Hotel stay",           date: "2026-01-20", category: "entertainment" },
    { bucketId: bucketByName["Amex Gold"],      amount:  84899, flow: "in",  note: "Payment",              date: "2026-02-01", category: "transfer" },
    // Wallet Cash
    { bucketId: bucketByName["Wallet Cash"],    amount:  20000, flow: "in",  note: "ATM withdrawal",       date: "2026-01-08", category: "transfer" },
    { bucketId: bucketByName["Wallet Cash"],    amount:   1200, flow: "out", note: "Coffee",               date: "2026-01-09", category: "food & dining" },
    // Coinbase
  ...
  for (const entry of mockEntries) {
    const exists = db.select({ id: ledger.id }).from(ledger)
      .where(sql`bucket_id = ${entry.bucketId} AND note = ${entry.note} AND date = ${entry.date}`).get();
    if (!exists) {
